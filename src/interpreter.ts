import { ENV, VALUE, UNDEFINED, STATEMENT, NUMBER, STRING, FunctionReturnValue, BLOCK_STATEMENT, BOOLEAN, LAMBDA, OBJECT } from './types';
...
function evaluate(statement: STATEMENT, env: ENV): VALUE {
    switch (statement.type) {
        case 'FUNCTION_DECLARATION': {
            env.bindings.set(statement.name, LAMBDA(statement.body, statement.params, env));
            return UNDEFINED;
        }
        case 'FUNCTION': {
        ...
        case 'CALL':
            ...
            return call(lambda.body, {
                parent: lambda.env,
                bindings: new Map(lambda.params.map((arg, i): [string, VALUE] => [arg, evaluate(statement.args[i], env)])),
            });
        case 'RETURN':
            throw new FunctionReturnValue(statement.rhs ? evaluate(statement.rhs, env) : UNDEFINED);
        case 'REFERENCE':
